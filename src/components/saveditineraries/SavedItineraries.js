...
const SavedItineraries = () => {
  ...
  function getItineraries() {
    fetch("http://localhost:3001/api/v1/itineraries")
        ...
        {itineraries.length > 0 ? (
          itineraries.map((itinerary) => ( 
            <article key={itinerary.id} className="saved-single-itinerary"> 
              <h3 className="saved-itinerary-title"> Itinerary for {itinerary.attributes.city} - {itinerary.attributes.duration} </h3>
